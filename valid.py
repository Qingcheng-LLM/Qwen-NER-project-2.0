...
def valid(model, tokenizer, dev_loader):
    ...
    with torch.no_grad():
        for batch in tqdm(dev_loader, desc="Valid (generate)"):
            ...
            gen_ids = model.predict (input_ids=input_ids,attention_mask=attention_mask,eos_token_id=tokenizer.eos_token_id,pad_token_id=tokenizer.eos_token_id,max_new_tokens=128,do_sample=False,)
